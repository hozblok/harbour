...
export class FAQStore {
    static URL = "https://stage.harbour.space/api/v1/scholarship_pages/data-science-apprenticeship-zeptolab";
    ...
    get types() {
        return Array.from(new Set(this.items.map(el => {
            return el.type;
        })));
    }
    ...
    async init(force = false) {
        if (this.initialized === false || force) {
            ...
            try {
                ...
                const items = resp.data.scholarship.faqs.items.map(({ type, question, answer }: FAQItems) => {
                ...
                alert(`Cannot get FAQ data from ${FAQStore.URL}`)
            }
        }
    }
}